...
def update_sampling_epsilon(config, global_model_step, scheduling_constant):
    """ Updates the hyper-parameter guiding the scheduled sampling process using the inverse sigmoid decay schedule;
    teacher-forcing is more likely for values closer to 1.0. See 'Scheduled Sampling for Sequence Prediction with
    Recurrent Neural Networks' for reference. """

    def _calculate_scheduling_constant():
        """ Helper function for determining the scheduling constant k. """
        logging.info('Calculating best scheduling constant for scheduled sampling. This might take a few seconds.\n')
        half_time = config.max_updates // 2
        lower_k_bound = 1
        upper_k_bound = config.max_updates
        best_divergence = 0.5
        best_k = None
        for k in range(lower_k_bound, upper_k_bound):
            with np.warnings.catch_warnings():
                # Suppress overflow warnings
                np.warnings.filterwarnings('ignore', r'overflow encountered')
                half_time_epsilon = k / (k + np.exp(half_time / k))
                divergence = np.abs(0.5 - half_time_epsilon)
    ...
    if scheduling_constant == 0:
        scheduling_constant = _calculate_scheduling_constant()
    epsilon = scheduling_constant / (scheduling_constant + np.exp(global_model_step / scheduling_constant))
    return epsilon, scheduling_constant
...
def train(config, sess_config):
    ...
    logging.info('-' * 20)
    model_size, mbyte_per_batch, gbyte_per_batch = count_parameters(config, model.float_dtype)
    logging.info('Number of model parameters (without activations): {:d}'.format(int(model_size)))
    if mbyte_per_batch is not None:
        logging.info('This corresponds to an approximate memory load of {:.2f}MB or {:.2f}GB'
                     .format(mbyte_per_batch, gbyte_per_batch))
    logging.info('-' * 20)
    ...
    for epoch_id in range(progress.eidx, config.max_epochs):
        ...
        while True:
            try:
                ...
                if not config.teacher_forcing_off:
                    if config.scheduled_sampling:
                        sampling_epsilon, scheduling_constant = \
                            update_sampling_epsilon(config, model_global_step, scheduling_constant)
                    else:
                        sampling_epsilon = 1.0
                ...
                feed_dict = {iterator_handle: train_handle,
                             ...
                             model.sampling_epsilon: sampling_epsilon,
                             model.training: True}
                ...
                def sample_model_output(random_sample=False, beam_search=False, n_displayed=10):
                    ...
                    # Surface a random batch
                    # instances = zip(*collected_fetches[np.random.randint(0, len(collected_fetches))])
                ...
                if config.valid_freq and model_global_step % config.valid_freq == 0:
                    ...
                    if config.bleu_script is not None:
                        # Re-initialize the validation iterator
                        sess.run(valid_init_op)
                        decoding_ops = [greedy_translations_op, beam_translations_op, beam_scores_op]
                        validation_bleu = \
...
def validation_bleu_loop(sess, model, config, ops, handles, target_dict, valid_summary_writer, valid_global_step,
    ...
    process_args = [config.bleu_script, temp_translation_file.name, config.valid_target_dataset]
...
def translation_loop(sess, ops, feed_dict, target_dict, out_file, external=False, beam_decoding=False, full_beam=False):
    """ Iterates over the translation source, generating translations in the target language. """
    # Unpack OPs
    greedy_trans_op, beam_trans_op, beam_scores_op = ops
    ...
    while True:
        try:
            if beam_decoding:
                target_batch, scores = sess.run([beam_trans_op, beam_scores_op], feed_dict=feed_dict)
            else:
...
def translate(config, sess_config, model=None):
    ...
    if config.num_gpus >= 2:
        translation_ops = \
            get_parallel_ops(model, translate_iterator, config.num_gpus, source_to_index['<EOS>'], 'translation')
        logging.info('[Parallel translation -- !! NO ORDER PRESERVATION !!]')
    else:
        translation_ops = \
    ...
    translation_loop(sess,
                     [greedy_translations_op, beam_translations_op, beam_scores_op],
                     feed_dict,
                     index_to_target,
                     target_file,
                     external=True,
